...
TcpEndpoint::TcpEndpoint(const Name &destination) : socket(STREAM) {
  sockaddr_in dst = destination;
  if (connect(this->socket, (sockaddr *)&dst, sizeof(dst)) < 0)
    throw SocketException(errno);
}

TcpEndpoint::TcpEndpoint(int fileDescriptor) : socket(fileDescriptor) {}

TcpEndpoint::~TcpEndpoint() { shutdown(this->socket, SHUT_RDWR); }
...
Name TcpEndpoint::getRemoteName() const {
  ...
  if (getsockname(this->socket, (sockaddr *)&name, &size) < 0)
    throw SocketException(errno);
  return Name(name);
}
...
Name TcpEndpoint::getLocalName() const {
  sockaddr_in name;
  socklen_t size = sizeof(name);
  if (getpeername(this->socket, (sockaddr *)&name, &size) < 0)
    throw SocketException(errno);
  return Name(name);
}
