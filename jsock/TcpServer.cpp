...
TcpServer::TcpServer(unsigned int port) : socket(STREAM) {
  ...
  if (bind(this->socket, (sockaddr *)&address, sizeof(address)) < 0)
...
std::unique_ptr<Endpoint> TcpServer::accept() const {
  ...
  int fileDescriptor = ::accept(this->socket, (sockaddr *)&address, &length);
  if (fileDescriptor < 0)
    if (errno != EAGAIN)
      if (errno != EWOULDBLOCK)
        throw SocketException(errno);
  return std::unique_ptr<Endpoint>(
      fileDescriptor < 0 ? NULL : new TcpEndpoint(fileDescriptor));
}
